...
const CreateParkingAreaScreen = () => {
  ...
  function validateEntries() {
    ...
    return name.length > 0 && spots > 0 && formValues.public.length > 0;
  }
        ...
        .then(({ results }) => {
          ...
          if (formValues.address.length > 0 && latLng === undefined) {
            ...
            setFormValues({
              ...
              address:
                formattedAddr !== formValues.address
                  ? formattedAddr
                  : formValues.address,
            });
          }
  ...
  return (
    <SafeAreaView>
      <ScrollView bounces={false}>
        ...
        >
          Please note that selecting 'Use Current Location' will reset all other
          values. {"\n"}
        </Text>
        <View>
          <View
          ...
          >
            ...
            >
              <Text style={{ fontSize: 16 }}>Use Current {"\n"} Location</Text>
            </TouchableOpacity>
          </View>
          <Text style={{ color: "#ffad00", fontStyle: "italic" }}>
            Refrain from adding unit/apartment numbers. {"\n"}
            <Text style={{ fontWeight: "bold" }}>
              Please double-check for correct address; input and detected
              address may not be the same.
            </Text>
          </Text>
          <FormTextField
        ...
        <View style={styles.review}>
          <Text style={styles.reviewTitle}>Review All Information</Text>
          ...
          <Text>
            <Text style={{ fontWeight: "bold" }}>Show on Google Maps:</Text>{" "}
            {String(formValues.hideFromMaps)}
          </Text>
        </View>
            ...
            fetch(`http://${WEBSITE}/parkingarea/`, {
              ...
              .then(async (res) => {
                try {
                  ...
                  Alert.alert(
                    "Created parking area!\nRefresh app to see parking info."
                  );
                  console.log(payload);
                } catch (err) {
                  Alert.alert(
                    `Parking area was not made. \nError: ${isError}\nMessage: ${message}`
                  );
                  console.error(err);
                }
              })
