...
const MarkerInfoScreen = ({ route, navigation }: any) => {
  ...
  const RedirectButton = (props: { link: string }) => {
    ...
    return (
      <Button mt="2" colorScheme="purple" onPress={handleRedirect}>
        Start Driving
      </Button>
    );
  };
  ...
  return (
    <SafeAreaView>
      <ScrollView bounces={false}>
        ...
        <RedirectButton link={GOOGLE_MAPS_REDIRECT} />
        <View style={styles.info}>
          <Text style={styles.t}>{parkingType + "\n"}</Text>
          {desc.length > 0 && <Text style={styles.t}>{desc + "\n"}</Text>}
          <Text style={styles.t}>Spots: {spots}</Text>
        </View>
